...
QString FeatureGraphicsService::generateMixedHeatmap(QString protName,
                                                     Matrix<float> *data,
    ...
    if(rFile.open(QIODevice::WriteOnly)){
        ...
        for(unsigned i = 0; i < data->rows(); ++i){
            for(unsigned j = 0; j < data->cols(); ++j){
                stream<<data->valueAt(i, j);
                stream << ", ";
                stream<<dataAux->valueAt(i, j);
                if (j < dataAux->cols() - 1) stream << ", ";
            }
            if (i < data->rows() - 1) stream << ", ";
        }

        stream << "), nrow = " << data->rows() << ", ncol = " << data->cols() * 2;
        stream << ", byrow = T, dimnames = list( residueA = NULL, residueB = NULL) )" << endl;
